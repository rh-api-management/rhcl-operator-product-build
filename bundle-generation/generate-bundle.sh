...
for env in dev stage prod; do
    ...
    yq -i '.metadata.labels["operatorframework.io/arch.s390x"] = "'"$(yq '.architectures.s390x' "$RHCL_CONFIG")"'"' "${CSV_FILE}"
    yq -i '.metadata.labels["operatorframework.io/arch.ppc64le"] = "'"$(yq '.architectures.ppc64le' "$RHCL_CONFIG")"'"' "${CSV_FILE}"
